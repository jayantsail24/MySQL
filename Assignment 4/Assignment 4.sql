...
select department, 
min(performance_score) `min score`
from emp
group by department
...
select year(hire_date) `hire year`, 
avg(performance_score) `average`
from emp
group by `hire year`
having `average` > 75
order by `hire year`;
...
select year(hire_date) `hire year`, 
count(employee_id) `count of employee`
from emp
group by `hire year`
...
select department, 
sum(salary + bonus) `total compensation`
from emp
group by department
...
select department, 
round(avg(performance_score), 2) `avg score`
from emp
group by department
having `avg score` > 8
order by sum(salary);
...
select year(hire_date) `hire years`,
min(salary) `min salary`,
round(avg(performance_score), 2) `avg score`
from emp
group by year(hire_date)
...
round(avg(performance_score), 2) `avg_score`
from emp
group by department
having `avg_score` < 80
...
select department,
sum(performance_score) `total score`,
...
round(avg(performance_score), 2) `average score`
from emp
group by department
...
select department,
min(performance_score) `minimum score`,
sum(salary) `total salary`
from emp
group by department
